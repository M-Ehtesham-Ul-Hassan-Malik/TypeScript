...
const fetchData = async (data) => {
    ...
    return result; // Return the parsed result as ExchangeRateApiResponse
};
// Function to fetch the conversion rate between two currencies
const fetchConversionRate = async (data) => {
    const response = await fetch(data); // Fetch data from the API
    const result = await response.json(); // Parse the JSON response
    const conversionRateResponse = result; // Cast the result to ConversionRateResponse
